...
public class TcpServer {
    ...
    public void startServer() {
        try (var serverSocket = new ServerSocket(Message.PORT)) {
            ...
            throw new CommunicationServiceException("error connecting clients", e);
        }
    }
...
//                System.out.println(request.getBody());
                ...
                Message response = methodHandlers.get(request.getHeader())
                        .apply(request);
//                common.Message response = new common.Message();
                response.writeTo(os);

            } catch (IOException e) {
                throw new CommunicationServiceException("error processing client", e);
            }
        }
    }
}
